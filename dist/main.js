...
/* harmony import */ var _libs_dayjs_min_js__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_libs_dayjs_min_js__WEBPACK_IMPORTED_MODULE_1__);


window.$ = (_libs_jquery_3_6_0_min__WEBPACK_IMPORTED_MODULE_0___default());
window.dayjs = (_libs_dayjs_min_js__WEBPACK_IMPORTED_MODULE_1___default());
var DEPARTMENTS = {
  CHATS: "CHATS",
  MA: "MA",
  ADMIN: "ADMIN"
...
window.Pontica = {
  ...
  exec: function exec(chromeStorage) {
    chromeStorage.local.get(null, function (items) {
      var state = items.checkboxesState;
      if (!state) chromeStorage.local.set({
      ...
      if (window.location.href.includes("https://stuart-team.atlassian.net/jira/servicedesk/projects/CSPUK/queues/custom/") && state[9]) {
        RTWJiraLinks();
      }
    });
  }
...
function MASidebarMod() {
  ...
  function main() {
    ...
    sortedDriverDistances.forEach(function (dist, i) {
      return markIfAssigned(dist, i, assignedDrivers, newTbody, selfAssignedDriversIds);
    });
  }
  ...
  function sortDriversByDistance(eligibleDriversTbody, newTbody) {
    if (eligibleDriversTbody) {
      ...
      for (var _i = 0; _i < sortedDriverDistances.length; _i++) {
        var desiredValue = sortedDriverDistances[_i];
        var indexInOldArr = driverDistances.indexOf(desiredValue);
        var newTableRow = eligibleDriversRows[indexInOldArr];
  ...
  function sidebarHourToEnglishTime() {
    ...
    var orderDay = date[0].substr(date[0].length - 2, date[0].length);
    var orderMonth = date[1];
    var orderYear = date[2].split("-")[0].trim();
    ...
    var match = rowTimingTd.innerHTML.split("-")[1].match(datesRegex);
    var stringToReplace = match[0];
    var hour = stringToReplace.split(":")[0];
    var minutes = stringToReplace.split(":")[1];
    hour = +hour;
...
function IntercomSlackConnection(items, chromeStorage) {
  ...
  items.colorCounter === undefined ? chromeStorage.local.set({
    colorCounter: 0
  }) : null;
  /* Executes in Slack */
  ...
  if (window.location.href.indexOf("slack") !== -1) {
     ...
     * @returns {null}
     */
    var crawlSlackChannel = function crawlSlackChannel(allMessages, manualChannelMessages, chatReferences, chatRefsColors, chatDeliveryRequests) {
      for (var i = 0; i < chatReferences.length; i++) {
        var cR = chatReferences[i];
        var dR = chatDeliveryRequests[i];

        for (var j = 0; j < allMessages.length; j++) {
          var foundPackageRefAtIndex = allMessages[j].innerHTML.indexOf(cR);
          var foundDeliveryReqAtIndex = allMessages[j].innerHTML.indexOf(dR);
        ...
        for (var k = 0; k < manualChannelMessages.length; k++) {
          var _foundPackageRefAtIndex = manualChannelMessages[k].innerHTML.indexOf(cR);

          var _foundDeliveryReqAtIndex = manualChannelMessages[k].innerHTML.indexOf(dR);

          if (_foundPackageRefAtIndex !== -1 || _foundDeliveryReqAtIndex !== -1) {
    ...
    setInterval(function () {
      ...
      var chatDeliveryRequests = Object.values(chatRefs).map(function (arr) {
        return arr[2];
      });
  ...
  function getMyChats() {
    ...
    for (var i = 0; i < list_chats.length; i++) {
      var currentChat = list_chats[i];

      for (var j = 0; j < chatRefsChatIds.length; j++) {
  ...
  function extendedChatHighlighting() {
    var _document$querySelect, _document$querySelect2, _document$querySelect3, _document$querySelect4, _packageReference, _deliveryRequest;
    ...
    if (chatReference && !chatRefs[chatReference] && anyPost()) {
      ...
      chromeStorage.local.set({
        chatRefs: JSON.stringify(chatRefs)
      });
      ...
      for (var i = 0; i < openedChatInfoMessages.length; i++) {
        for (var j = 0; j < intercomToSlackMessages.length; j++) {
          if (openedChatInfoMessages[i].innerHTML.includes(intercomToSlackMessages[j])) {
            var _currentChatId = window.location.href.match(/conversations\/(\d+)/)[1];
            chatRefs[chatReference] = [_currentChatId, chatColors[counter], deliveryRequest];
            chromeStorage.local.set({
  ...
  function resetValues() {
    ...
    if (items.colorCounter >= chatColors.length - 1) {
      ...
      chromeStorage.local.set({
        colorCounter: items.colorCounter + 1
      });
    }
  }
...
function BOFilterPackages() {
  var table = document.getElementById("index_table_packages");
  var indexFooter = document.getElementById("index_footer");
  var scopeGroup = document.getElementsByClassName("scope-default-group")[0];
  var multipleChoiceForm = document.createElement("form");
  ...
  if (table) {
    ...
    multipleChoiceForm.addEventListener("submit", function (e) {
  ...
  function openLinks(rows, type) {
    for (var i = 0; i < rows.length; i++) {
      var _currentRowStatus$inn;

      var currentRow = rows[i];
      var currentRowStatus = rows[i].getElementsByClassName(type)[0];

      if ((currentRowStatus === null || currentRowStatus === void 0 ? void 0 : (_currentRowStatus$inn = currentRowStatus.innerHTML) === null || _currentRowStatus$inn === void 0 ? void 0 : _currentRowStatus$inn.toLowerCase()) != type) {
        continue;
      }

      var currentRowId = rows[i].getElementsByClassName("col-id")[0].getElementsByTagName("a")[0];
      var href = currentRowId.getAttribute("href");
...
function RTWJiraLinks() {
  ...
  var oldUrl = "";
  setInterval(function () {
    var headerField = document.querySelector("h1[data-test-id='issue.views.issue-base.foundation.summary.heading']");
    ...
    if (headerField && !hasPassed) {
      var headerText = headerField.innerText;
      var driverId = headerText.match(/\d{6}/g)[0];
      var email = headerText.match(/\w+@\w+.\w+/g)[0];
      var fountainLink = document.createElement("a");
